...
BookSnapshot BookSnapshot::from_book(const lob::Book& book, const std::string& sym,
    ...
    auto& buy_limits = const_cast<lob::Book&>(book).get_buy_limits();
    snap.bids.reserve(buy_prices.size());
    for (lob::PRICE p : buy_prices) {
        auto it = buy_limits.find(p);
        if (it != buy_limits.end()) {
            lob::Level* lvl = it->second;
            snap.bids.push_back({
    ...
    auto& sell_limits = const_cast<lob::Book&>(book).get_sell_limits();
    snap.asks.reserve(sell_prices.size());
    for (lob::PRICE p : sell_prices) {
    ...
    snap.mid_price = book.get_mid_price() > 0
                         ? converter.to_external(static_cast<lob::PRICE>(book.get_mid_price()))
                         : 0.0;
